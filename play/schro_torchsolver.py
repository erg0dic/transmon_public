...
def get_init_states(ham_rank) -> Tuple[torch.Tensor, np.ndarray]:
    u_0 = np.array([np.eye(ham_rank), 0*np.eye(ham_rank)]).reshape(ham_rank*2,ham_rank)
...
if __name__ == '__main__':
    ...
    def test_random_Hamiltonian_evolutions(num_hamiltonians=4, final_time=10, terms_per_ham=4, 
                                        freq_low=0., freq_high=5., epss=[1e-1, 1e-2], 
                                        seed=2, alpha=1, single_H=True, 
                                        plot_cnot_traj_proj=False, figax: Tuple = None, normcols=None):
        ...
        def test_Ham(t, H_terms, trig_choices, random_freqs, coeffs):
            ...
            for term in range(terms_per_ham):
                if Ham is None:
                    Ham = coeffs[term]*trigs[trig_choices[term]](random_freqs[term]*np.pi*t)*H_terms[term]
                else:
                    Ham += coeffs[term]*trigs[trig_choices[term]](random_freqs[term]*np.pi*t)*H_terms[term]
            return Ham
        def H_test_torch_fn(t, H_terms, trig_choices, random_freqs, coeffs):
            ...
            H_test_torch = augmented_H(H_test_torch_real, H_test_torch_imag, cont_part=0)
            return H_test_torch
        ...
        if num_hamiltonians%2 != 0:
            ...
            col_inc = 0
        if not figax:
            fig, ax = plt.subplots(nrows=2, ncols=num_hamiltonians//2+col_inc, figsize=(14,14))
        else:
            fig, ax = figax
        ax = ax.ravel()
        ...
        for plt_num,n_qubit in enumerate(num_qubits):
            ...
            random_freqs = np.random.uniform(low=freq_low, high=freq_high, size=terms_per_ham)
            trig_choices = np.random.randint(low=0, high=2,size=terms_per_ham)
            coeffs = np.random.uniform(low=-freq_high, high=freq_high, size=terms_per_ham)
            ...
            if single_H:
                ...
                for i in range(terms_per_ham):
                    if trig_choices[i]== 0:
                        ...
                        tr = "s"
                    rand_Ham_string += f"{round(coeffs[i],2)}"+SU2_basis[H_terms_ind[i]]+ f"{tr}({round(random_freqs[i], 2)}" + r"$t$)"
                    if i < terms_per_ham-1:
                        rand_Ham_string += " + "

            for eps, ceps, normcol in zip(epss, colors[:len(epss)], normcols[:len(epss)]):
                ...
                for i, t in enumerate(times):
                    H_test = test_Ham(t, H_terms, trig_choices, random_freqs, coeffs)
                    schro_single_step_evolver = lambda t,x,c: H_test_torch_fn(t, H_terms, 
                                                                        trig_choices, random_freqs, coeffs)@x
                    test_u_0 = expm(-1j*H_test*final_time/steps)@test_u_0
                    intermediate_u = evolve_Heun(t, u_0, step=eps, f=schro_single_step_evolver)
                    u_0 = intermediate_u
                ...
                ax[plt_num].semilogy(times, frobnorms, label=f"res = {eps}", 
                                     alpha=alpha, color=normcol)
                if plt_num != 0 and plt_num != 1:
                    ax[plt_num].set_xlabel("time", fontsize=15)
                ax[plt_num].set_ylabel(r"$\| U_{Heun} - U_{exp} \|_F^2$", fontsize=15)
                if single_H:
                    ax[plt_num].set_title(f"n={n_qubit}, H={rand_Ham_string}", fontsize=11)
                else:
                    ax[plt_num].set_title(f"n={n_qubit}", fontsize=11)
                if plot_cnot_traj_proj and eps==epss[-1]:
                    ax2.plot(times, cnot_fids, alpha=alpha*0.5, label=f"nominal trajectory with eps={eps}", color=ceps)
    ...
    def fid_torch(U, U_tgt, HAM_RANK=HAM_RANK):
        ...
        U_dag_Utgt_real = U[:HAM_RANK].T @ U_tgt[:HAM_RANK] + U[HAM_RANK:].T @ U_tgt[HAM_RANK:]
        U_dag_Utgt_imag = U[HAM_RANK:].T @ U_tgt[:HAM_RANK] - U[:HAM_RANK].T @ U_tgt[HAM_RANK:]
        trU_dagU_tgt = torch.trace(U_dag_Utgt_real)**2 + torch.trace(U_dag_Utgt_imag)**2
    ...
    def frobnorm(U, U_tgt):
        ...
        return ((U[:HAM_RANK]-U_tgt[:HAM_RANK])**2).sum() + ((U[HAM_RANK:]-U_tgt[HAM_RANK:])**2).sum()
    ...
    def opt_schro(final_time=1, eps=1e-2, u_tgt: np.ndarray =None, debug=False, epochs=500):
        u_0 = np.array([np.eye(HAM_RANK), 0*np.eye(HAM_RANK)]).reshape(HAM_RANK*2,HAM_RANK)
        u_0 = torch.as_tensor(u_0, dtype=torch.float32)
        if u_tgt is None:
            if HAM_RANK == 2:
                ...
                raise Exception(f"Please provide a `u_tgt` as a default gate is not available for {N_Q} qubits")
            u_tgt = np.array([TARGET, 0*np.eye(HAM_RANK)]).reshape(HAM_RANK*2,HAM_RANK)
        u_tgt = torch.as_tensor(u_tgt, dtype=torch.float32)
        u_tgt.requires_grad=False
        steps = int(final_time / eps)
        conts = nn.Parameter(torch.as_tensor(np.ones((steps))*0.1, dtype=torch.float32), requires_grad=True)
        ...
        for epoch in range(epochs):
            ...
            for i, t in enumerate(times):
                ...
                if debug:
                    ...
                    test_u_0 = expm(-1j*H_test*final_time/steps)@test_u_0
                ### debugging code ####
                intermediate_u = evolve_Heun(t, u_0, step=eps, f=schro_RHS_torch, c=conts[i])
                u_0 = intermediate_u
                if debug:
                    nominal_u_0 = (u_0[:HAM_RANK]+1j*u_0[HAM_RANK:]).detach().numpy()
            ...
            if epoch%50 == 0:
                ...
                if epoch > 200:
                    ax[0].plot(times, conts.detach().numpy(), label=r"$u(t)$"+f" at epoch {epoch}", c="red", alpha=0.4)
        # print(u_0)
        nominal_gate = u_0[:HAM_RANK]+1j*u_0[HAM_RANK:]
        print("nominal tensor is ", nominal_gate)
        print("the target was ", u_tgt)
        print("cost of this nominal gate is: ", fid(nominal_gate.detach().numpy(), TARGET))
    ...
    def compare_real_and_complex_evo():
        u_0 = np.array([np.eye(HAM_RANK), 0*np.eye(HAM_RANK)]).reshape(HAM_RANK,HAM_RANK*2)
        final_time = 1
        eps = 1e-2
        steps = int(final_time / eps)
        times = np.linspace(0, final_time, steps)
    ...
    def test_scipy_ode_solver():
        ...
        sol = solve_ivp(schro_RHS, [0,1], np.eye(HAM_RANK, dtype=np.complex128).ravel(), t_eval=np.linspace(0,1,10) )
        print("complex ode solver", sol.y[:,-1].reshape(HAM_RANK,HAM_RANK))

    def plot_multiple_seeds_for_heun_test():
        figax = None; alpha=1.
        for seed in [1,2,3,4,5]:
            figax = test_random_Hamiltonian_evolutions(single_H=False, plot_cnot_traj_proj=False, seed=seed, figax=figax, alpha=0.4, normcols=["blue", "orange"])

    # opt_schro(debug=True, eps=1e-2, final_time=2, epochs=300)
    test_random_Hamiltonian_evolutions(single_H=True, plot_cnot_traj_proj=True, seed=7,alpha=1)
