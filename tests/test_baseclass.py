...
import unittest
import numpy as np
from src.utilities import random_density_matrix, dm2vec, vec2dm, get_pauli_basis_matrices
class TestPaulis(unittest.TestCase):
    ...
    def test_double_pauli_multiplication(self):
        sigma_1 = "XYZY1"
        sigma_2 = "ZY1XX"
        self.assertTrue(self.qsys.multiply_general_paulis(sigma_1, sigma_2) == "_-1_Y1ZZX")
        sigma_1 = "_-1j_XYZY1"
        sigma_2 = "_-1j_ZY1XX"
        self.assertTrue(self.qsys.multiply_general_paulis(sigma_1, sigma_2) == "Y1ZZX")
        sigma_1 = "_-1j_XYZY1"
        sigma_2 = "ZY1XX"
        self.assertTrue(self.qsys.multiply_general_paulis(sigma_1, sigma_2) == "_1j_Y1ZZX")
        sigma_1 = "XYZY1"
        sigma_2 = "_-1j_ZY1XX"
        self.assertTrue(self.qsys.multiply_general_paulis(sigma_1, sigma_2) == "_1j_Y1ZZX")
        sigma_1 = "XYZY1"
    ...
    def test_multiple_pauli_multiplication(self):
        ...
        pauli_list = [sigma_1, sigma_1, sigma_2, sigma_2, sigma_1]
        self.assertTrue(self.qsys.multiply_many_general_paulis(pauli_list) == "_1j_XYZY1")
        ...
        sigma_1 = "XYZY1"
        sigma_2 = "ZY1XX"
        self.assertTrue(self.qsys.multiply_many_general_paulis([sigma_1, sigma_2]) == "_-1_Y1ZZX")
    ...
    def test_pauli_basis_generators(self):
        ...
        for s1 in ["1", "X", "Y", "Z"]:
            for s2 in ["1", "X", "Y", "Z"]:
                A.add(s1+s2)
        self.assertTrue(len(list(A.intersection(set(paulis_2qubits)))) == len(paulis_2qubits))
    ...
    def test_pauli_commutator(self):
        ...
        self.assertTrue(np.allclose(self.qsys.pauli_commutator("X", "Y"), 
                                2j*self.qsys.get_pauli["Z"]))
        self.assertTrue(np.allclose(self.qsys.pauli_commutator("Y", "Z"), 
                                2j*self.qsys.get_pauli["X"]))

        self.assertTrue(np.allclose(self.qsys.pauli_commutator("Z", "X"), 
                                2j*self.qsys.get_pauli["Y"]))
    ...
    def test_integration_1(self):
        ...
        for i in range(len(randis[0])):
            ...
            self.assertTrue(np.allclose(self.qsys.pauli_anticommutator(p1, p2), xx_p_yy))
    ...
    def test_density_mat_generation_batch(self):
        for way in ["Bures", "HS"]:
            for dim in range(2,17):
                ...
                self.assertTrue(np.allclose(mat1.conj().transpose(-3,-1,-2)-mat1, 0))
                self.assertTrue(np.allclose(mat2.conj().transpose(-3,-1,-2)-mat2, 0))
    ...
    def test_two_qubit_3_param_family(self):
        X = QSys.get_pauli["X"]; Y = QSys.get_pauli["Y"] 
        XX = np.kron(X, X)
        YY =  np.kron(Y, Y)
        expected = expm(-1j*XX); expected2 = expm(-1j*(XX+YY)) 
