...
class TestTransmon(unittest.TestCase):
    ...
    def test_ham_hermitian(self):
        self.assertTrue(np.allclose(self.t.two_level_hamiltonian(), 
                        self.t.two_level_hamiltonian().T.conjugate()), 
                        "2 lvl Ham not physical!")
        self.assertTrue(np.allclose(self.t.hamiltonian_full(), 
                        self.t.hamiltonian_full().T.conjugate()), 
                        "2 lvl Ham not physical!")
    ...
    def test_lindblad_correctness(self):
        ...
        for rho in rhos:
            assert np.allclose(rho.T.conjugate(), rho), "lindblad evolution is not hermiticty preserving"
    ...
    def test_ode_solver_for_lindblad_evo(self):
        ...
        # an iffy test...
        assert (np.real(fids1[mask])-np.real(fids2[mask])).sum() <= 0.5, "fids non matching approximately"
