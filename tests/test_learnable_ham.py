...
def test_sgd_of_learnable_ham(batch_size=10, heun_eps=1e-2, opt_lr=1e-3):
    ...
    learner = LearnableHamiltonian(
        4, qubits=2, num_timesteps=20, debug=False, heun_eps=heun_eps, ansatz_A=True
    )
    ...
    for t in range(10000):
        out_Us, _ = learner.predict_prop(
            torch.as_tensor(batch_acts, dtype=torch.float32), randUs
        )
...
class TestUnitaryHamLearner(unittest.TestCase):
    ...
    learner = LearnableHamiltonian(
        1, qubits=2, num_timesteps=20, debug=True, heun_eps=1e-4, dont_batch_inputs=True
    )
    hetero_learner = LearnableHamiltonian(
        4, qubits=2, num_timesteps=20, debug=False, heun_eps=1e-4, ansatz_A=True
    )
    ...
    def test_single_propagation_step_unitary(self):
        ...
        for _ in range(5):
            ...
            # get complex unitary
            out_state = (
                out_state[: len(out_state) // 2] + 1j * out_state[len(out_state) // 2 :]
            )
            out_state = out_state.reshape(int(np.sqrt(len(out_state))), -1)

            # check if the learner can get the same single step propagation
            out_target_state, rew_t = self.learner.predict_prop(
                torch.as_tensor(sample_action, dtype=torch.float32),
                torch.as_tensor(test_starting_state, dtype=torch.float32),
            )
            out_target_state, rew_t = (
                out_target_state.detach().numpy(),
                rew_t.detach().numpy(),
            )
            out_target_state_c = (
                out_target_state[: len(out_target_state) // 2]
                + 1j * out_target_state[len(out_target_state) // 2 :]
            )
    ...
    def test_uni_to_super_to_choi_conversion_for_multiple_qubits(self):
        for q in range(1, 5):
            for _ in range(100):
                ...
                assert np.allclose(
                    choi.T.conj(), choi, atol=1e-4
                ), "choi conversion unsuccessful"

    def test_super_op_choi_to_bloch_torch(self):
        for _ in range(10):
            rand_U = unitary_group.rvs(int(pow(2, self.learner.qubits)))
            super_target = super_op(rand_U)
            rand_U_torch = torch.as_tensor(
                np.array([np.real(rand_U), np.imag(rand_U)]).reshape(
                    2 * rand_U.shape[-1], -1
                ),
                dtype=torch.float32,
            )
            super_op_torch_real, super_op_torch_imag = self.learner.super_op(
                rand_U_torch
            )
            super_op_torch_real, super_op_torch_imag = (
                super_op_torch_real.detach().numpy(),
                super_op_torch_imag.detach().numpy(),
            )
            self.assertTrue(
                np.allclose(
                    super_target,
                    (super_op_torch_real + 1j * super_op_torch_imag),
                    atol=1e-4,
                )
            )
    ...
    def test_batch_fidelity_getter(self):
        ...
        batch_fids = (
            self.learner.unitary_fid(
                rand_Us, self.learner.target, self.learner.trl, self.learner.qubits
            )
            .detach()
            .numpy()
        )
        batch_fids_target = np.array(
            [self.tester.cost_function(rand_U, CNOT) for rand_U in rand_Us_t]
        )
    ...
    def test_super_op_choi_to_bloch_torch_batch(self):
        ...
        self.assertTrue(
            np.allclose(target_chois, (chRs + 1j * chIs).detach().numpy(), atol=1e-4)
        )
        target_bvecs = np.array(
            list(map(lambda x: dm2vec(x, self.learner.paulis_2n), Us2choi(rand_Us_t)))
        )
    ...
    def test_homogeneous_batch_ensemble_single_propagation_step_unitary(self):
        ...
        learner = LearnableHamiltonian(
            5, qubits=2, num_timesteps=20, debug=True, heun_eps=1e-4
        )
        self.tester.reset()
        test_starting_state, _ = get_init_states(int(pow(2, self.learner.qubits)))
        ...
        for _ in range(5):
            ...
            test_starting_state = np.tile(test_starting_state.ravel(), reps=20).reshape(
                20, *test_starting_state.shape
            )
            out_target_state, rew_t = learner.predict_prop(
                torch.as_tensor(sample_action, dtype=torch.float32),
                torch.as_tensor(test_starting_state, dtype=torch.float32),
            )
            ...
            out_target_state_c = (
                out_target_state[:, :, : out_target_state.shape[-2] // 2, :]
                + 1j * out_target_state[:, :, out_target_state.shape[-2] // 2 :, :]
            )
            self.assertTrue(
                np.allclose(
                    out_state,
                    out_target_state_c.sum(axis=0).sum(axis=0) / (5 * 20),
                    atol=1e-3,
                )
            )
    ...
    def test_semiheterogeneous_batch_ensemble_single_propagation_step_unitary(self):
        ...
        (
            unitary_ensemble_prop_batch,
            bloch_ensemble_prop_batch,
            rews,
        ) = self.get_training_data(learner, batch_init_states, batch_acts)
    ...
    def test_lindblad_diss_correctness(self):
        ...
        diss_params = torch.as_tensor(
            np.real(col_ops), dtype=torch.float32
        ), torch.as_tensor(np.imag(col_ops), dtype=torch.float32)
        ...
        self.assertTrue(
            np.allclose(t_lind_term, my_lind_term.detach().numpy(), atol=1e-6)
        )
    ...
    def test_heterogeneous_batch_ensemble_single_propagation_step_unitary(self):
        ...
        tbatch_init_states = tbatch_init_states.reshape(
            4, 100, -1, int(pow(2, learner.qubits))
        )
        batch_init_states = batch_init_states.reshape(
            4, 100, -1, int(pow(2, learner.qubits))
        )
        (
            unitary_ensemble_prop_batch,
    ...
    def test_choi_to_bloch_to_choi_batch(self):
        ...
        supers = torch.cat(
            learner.convert_to_choi(chois_ri, dont_convert_to_super=True), dim=-2
        )
    ...
    def test_heterogeneous_batch_ensemble_single_propagation_step_lind(self):
        ...
        tbatch_init_states = tbatch_init_states.reshape(
            4, 10, -1, int(pow(2, learner.qubits))
        )
        batch_init_states = batch_init_states.reshape(
            4, 10, -1, int(pow(2, learner.qubits))
        )
        unitary_ensemble_prop_batch, _, _ = self.get_training_data(
            learner, batch_init_states, batch_acts, batch_size=10
        )
        tbatch_init_states = torch.cat(learner.super_op(tbatch_init_states), dim=-2)
        out_target_states, _ = learner.predict_prop(
            torch.as_tensor(batch_acts, dtype=torch.float32),
            tbatch_init_states,
            lind=True,
        )
        unitary_ensemble_prop_batch = np.concatenate(
            [
                np.real(unitary_ensemble_prop_batch),
                np.imag(unitary_ensemble_prop_batch),
            ],
            axis=-2,
        )
        unitary_ensemble_prop_batch = torch.as_tensor(
            unitary_ensemble_prop_batch, dtype=torch.float32
        )
        unitary_ensemble_prop_batch = torch.cat(
            learner.super_op(unitary_ensemble_prop_batch), dim=-2
        )
        out_target_states, unitary_ensemble_prop_batch = (
            out_target_states.detach().numpy(),
            unitary_ensemble_prop_batch.detach().numpy(),
        )
        self.assertTrue(
            np.allclose(out_target_states, unitary_ensemble_prop_batch, atol=1e-4)
        )
    ...
    def test_heterogeneous_batch_ensemble_entire_trajectory_unitary(self):
        ...
        for i_t in range(learner.num_timesteps):
            ...
            U_t_i_s, _, _ = self.get_training_data(
                learner, U_t_i_s, batch_acts_per_timestep, batch_size=2
            )
        self.assertTrue(np.allclose(U_t_i_s - final_Us, 0, atol=1e-3))

    def get_init_learner_and_acts(
        self, full_traj=False, batch_size=100, num_timesteps=20
    ):
        ...
        if not full_traj:
            ...
            batch_acts = (
                np.random.normal(size=(4, batch_size, num_timesteps, 2)) * 20 - 10
            ) % 10
    ...
    def get_training_data(
        ...
        unitary_ensemble_prop_batch = np.zeros(
            (4, batch_size, batch_init_states.shape[-1], batch_init_states.shape[-1]),
            dtype=np.complex128,
        )
        bloch_ensemble_prop_batch = np.zeros(
            (4, batch_size, int(pow(2, learner.qubits * 2 * 2)))
        )
        ...
        for i, sys in enumerate(learner.syses):
            ...
            unitary_ensemble_prop_batch[i] = np.array(
                list(
                    map(
                        lambda U, act: evolve_tester(sys, action=act, init_U=U),
                        batch_inits,
                        acts,
                    )
                )
            ...
            rews[i] = np.array(
                list(
                    self.tester.cost_function(evo_U, CNOT)
                    for evo_U in unitary_ensemble_prop_batch[i]
                )
            )
            bloch_ensemble_prop_batch[i] = np.array(
                list(
                    dm2vec(
                        self.tester.qsys.super_to_choi(super_op(evo_U)),
                        learner.paulis_2n,
                    )
                    for evo_U in unitary_ensemble_prop_batch[i]
                )
    ...
    def run_batch_tests(
        ...
        out_target_states, rew_ts = learner.predict_prop(
            torch.as_tensor(batch_acts, dtype=torch.float32), tbatch_init_states
        )
        out_blochs_targets = (
            learner.convert_to_bloch(out_target_states).detach().numpy()
        )

        out_target_states, rew_ts = (
            out_target_states.detach().numpy(),
            rew_ts.detach().numpy(),
        )
        out_target_states = (
            out_target_states[:, :, :4, :] + 1j * out_target_states[:, :, 4:, :]
        )
        ...
        self.assertTrue(
            np.allclose(out_blochs_targets, bloch_ensemble_prop_batch, atol=1e-4)
        )
